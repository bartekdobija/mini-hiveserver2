...
public class MiniHS2 extends AbstractHiveService {
  ...
  private static final FsPermission WRITE_ALL_PERM = new FsPermission(
      (short) 00733);
  ...
  public static class Builder {
    ...
    public Builder withMiniMR() {
      miniClusterType = MiniClusterType.MR;
      return this;
    }
    
    public Builder withMiniKdc(final String serverPrincipal,
        final String serverKeytab) {
      useMiniKdc = true;
    ...
    public Builder withConf(final HiveConf hiveConf) {
    ...
    public MiniHS2 build() throws Exception {
      ...
      return new MiniHS2(hiveConf, miniClusterType, useMiniKdc,
          serverPrincipal, serverKeytab, isMetastoreRemote);
    }
  }
  ...
  public void setMr(final MiniMrShim mr) {
    this.mr = mr;
  }
  ...
  public void setDfs(final MiniDFSShim dfs) {
    this.dfs = dfs;
  }
  ...
  public void setMiniClusterType(final MiniClusterType miniClusterType) {
  ...
  private MiniHS2(final HiveConf hiveConf,
      final MiniClusterType miniClusterType, final boolean useMiniKdc,
      final String serverPrincipal, final String serverKeytab,
      final boolean isMetastoreRemote) throws Exception {
    super(hiveConf, "localhost", MetaStoreUtils.findFreePort(), MetaStoreUtils
        .findFreePort());
    this.miniClusterType = miniClusterType;
    ...
    if (miniClusterType != MiniClusterType.DFS_ONLY) {
      ...
      String uriString = WindowsPathUtil.getHdfsUriString(fs.getUri()
          .toString());
    ...
    if (useMiniKdc) {
      hiveConf
          .setVar(ConfVars.HIVE_SERVER2_KERBEROS_PRINCIPAL, serverPrincipal);
    ...
    String metaStoreURL = "jdbc:derby:" + baseDir.getAbsolutePath()
        + File.separator + "test_metastore-" + hs2Counter.incrementAndGet()
        + ";create=true";
    ...
    // reassign a new port, just in case if one of the MR services grabbed the
    // last one
    ...
    // Create root scratchdir with write all, so that user impersonation has no
    // issues.
  ...
  public MiniHS2(final HiveConf hiveConf) throws Exception {
    this(hiveConf, MiniClusterType.DFS_ONLY);
  }
  
  public MiniHS2(final HiveConf hiveConf, final MiniClusterType clusterType)
      throws Exception {
    this(hiveConf, clusterType, false, null, null, false);
  }
  
  public void start(final Map<String, String> confOverlay) throws Exception {
    if (isMetastoreRemote) {
      int metaStorePort = MetaStoreUtils.findFreePort();
      getHiveConf().setVar(ConfVars.METASTOREURIS,
   ...
   * 
   * @param dbName
   *          - DB name to be included in the URL
   * @return
   */
   ...
   * @param dbName
   *          - DB name to be included in the URL
   * @param sessionConfExt
   *          - Addional string to be appended to sessionConf part of url
   * @return
   */
  public String getJdbcURL(final String dbName, final String sessionConfExt) {
   ...
   *          - Addional string to be appended to sessionConf part of url
   * @param hiveConfExt
   *          - Additional string to be appended to HiveConf part of url
   *          (excluding the ?)
   * @return
   */
  public String getJdbcURL(final String dbName, String sessionConfExt,
      String hiveConfExt) {
